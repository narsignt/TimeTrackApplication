...
// This is for session log creation by sunil
function saveAuditSessionLogin($user_id, $UserName)
{
  // This for session creation log when user login, it create one record when user enter to time track
...
{
  // This for session update the log when user logout, it oudates the login session record when user quit from time track
  //right now this is not using. We are not creating session to check session end
...
{
   // This method for actionon log when user perform the action it creates the row based session id	
